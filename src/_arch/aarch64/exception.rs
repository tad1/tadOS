...
impl KernelGate{
    pub const fn new(value: u64) -> Self {
        Self { inner: NullLock::new(KernelGateInner { function: value }) }
    }
}


static KERNEL_GATE: KernelGate = KernelGate::new(0);

pub unsafe fn set_kernel_gate(addr: u64){
    KERNEL_GATE.inner.lock(|inner| inner.function = addr);
}
...
// Public Code
//--------------------------------------------------------------------------------------------------
use crate::{exception::PrivilegeLevel, info, synchronization::{NullLock, interface::Mutex}};
